...
$servicePrjFlt = Get-Service -Name "prjflt" -ErrorAction SilentlyContinue

if ($servicePrjFlt.Count -eq 0)
{
    Write-Host "prjflt service does not exist"

    $featureProjFS = Get-WindowsOptionalFeature -FeatureName Client-ProjFS -Online

    if ($featureProjFS.State -eq "Enabled")
    {
        Write-Error "ProjFS does not exist but is enabled?!"
        return 1
    }
    ...
    else
    {
        ...
        Write-Host "Copying user-mode DLL into System32"
...
elseif ($servicePrjFlt.Status -eq "Stopped")
{
    Start-Service -Name "prjflt"
}

Write-Host "ProjFS should be available:"
Get-ChildItem $env:SystemRoot\System32\ProjectedFSLib.dll,$env:SystemRoot\System32\drivers\prjflt.sys 